...
class PartAwareAugmentation(object):
    def __init__(self, points, gt_boxes, gt_names, class_names=None, random_partition=False):
        ...
        self.aug_flag = np.zeros((self.num_gt_boxes, 8, 6), dtype=bool)

    def interpret_pa_aug_param(self, pa_aug_param):
        pa_aug_param_dict = {}
        method_list = ['dropout', 'sparse', 'noise', 'swap', 'mix', 'jitter', 'random', 'distance', 'ptmixup']
        for method in method_list:
            ...
            if method == 'ptmixup':
                ...
                pa_aug_param_dict[method + '_alpha'] = 1.0
                pa_aug_param_dict[method + '_beta'] = 1.0
                continue
            pa_aug_param_dict[method] = 0
            pa_aug_param_dict[method + '_p'] = 0
        ...
        for i, param in enumerate(param_list):
            ...
            for method in method_list:
                if param.startswith(method):
                    ...
                    if i + 1 < len(param_list) and param_list[i + 1].startswith('p'):
                    ...
                    if method == 'ptmixup':
                        if i + 2 < len(param_list):
                            alpha = param_list[i + 1].replace('a', "")
                            beta = param_list[i + 2].replace('b', "")
                            pa_aug_param_dict[method + '_alpha'] = float(alpha) / 10.0
                            pa_aug_param_dict[method + '_beta'] = float(beta) / 10.0
                    method_list.remove(method)
                    break
        return pa_aug_param_dict
    ...
    def point_mixup(self, alpha=1.0, beta=1.0, p=0.5, distance_limit=100):
        if np.random.rand() > p:
            return
        ...
        for i in range(self.num_gt_boxes):
            ...
            # Find objects of the same class
            same_class_mask = self.gt_names == self.gt_names[i]
            same_class_mask[i] = False  # Exclude the current object
            same_class_indices = np.where(same_class_mask)[0]

            if len(same_class_indices) == 0:
                continue  # No other objects of the same class

            # Randomly choose another object of the same class
            target_idx = np.random.choice(same_class_indices)

            # Mix points in each partition
            for j in range(len(self.separated_box_points[i])):
                points1 = self.separated_box_points[i][j]
                points2 = self.separated_box_points[target_idx][j]

                if len(points1) == 0 or len(points2) == 0:
                    continue  # Skip empty partitions

                num_points = min(len(points1), len(points2))
                mixed_points = lam * points1[:num_points] + (1 - lam) * points2[:num_points]

                self.separated_box_points[i][j] = mixed_points

            # Mix bounding box parameters
            self.gt_boxes[i] = lam * self.gt_boxes[i] + (1 - lam) * self.gt_boxes[target_idx]

        # Mix background points
        if len(self.bg_points) > 0:
            shuffled_bg_points = np.random.permutation(self.bg_points)
            mixed_bg_points = lam * self.bg_points + (1 - lam) * shuffled_bg_points
            self.bg_points = mixed_bg_points
    ...
    def augment(self, pa_aug_param):
        ...
        self.point_mixup(alpha=pa_aug_param['ptmixup_alpha'], beta=pa_aug_param['ptmixup_beta'], p=pa_aug_param['ptmixup_p'], distance_limit=pa_aug_param['distance'])
