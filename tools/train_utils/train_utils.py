...
def eval_single_ckpt(model, test_loader, args, ckpt_name, eval_output_dir, logger, epoch_id, dist_test=False, cfg=None, tb_log=None, val_loss=False):
    # load checkpoint

    # model.load_params_from_file(filename=ckpt_name, logger=logger, to_cpu=dist_test)
    if hasattr(model, 'module'):
        model.module.load_params_from_file(filename=ckpt_name, logger=logger, to_cpu=dist_test)
    else:
        model.load_params_from_file(filename=ckpt_name, logger=logger, to_cpu=dist_test)
    model.cuda()
...
def train_model(model, optimizer, train_loader, model_func, lr_scheduler, optim_cfg,
                ...
                , test_loader=None,  trainacc_loader=None,args=None):
        ...
        for cur_epoch in tbar:
            ...
            if trained_epoch % ckpt_save_interval == 0 and rank == 0:
                ...
                if test_loader:
                    ...
                    eval_single_ckpt(model, test_loader, args, cp_name, eval_output_dir, logger, trained_epoch, dist_test=False, cfg=cfg,tb_log=tb_log, val_loss=True)
                # if trainacc_loader: 
                ...
                #     eval_single_ckpt(model, train_loader, args, cp_name, eval_output_dir, logger, trained_epoch, dist_test=False, cfg=cfg,tb_log=tb_log)
